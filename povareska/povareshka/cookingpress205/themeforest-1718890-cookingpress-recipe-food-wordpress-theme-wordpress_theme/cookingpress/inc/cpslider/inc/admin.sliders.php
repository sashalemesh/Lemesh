    ...
    if ( wp_verify_nonce( $_REQUEST['_wpnonce'], 'new_slider' ) ) {
        ...
        if (  ! empty( $_POST['slider_name'] ) ) {
            ...
            if($exist == false) {
                ...
                $new_slider->save();
                $messages[] = '<div id="message" class="updated"><p>' . __('New slider ', 'cookingpress') . '<strong>' . $_POST['slider_name'] . '</strong>' . __(' has been successfully created', 'cookingpress') . '</p></div>';
            } else {
                 $messages[] = '<div id="message" class="error fade"><p>' . __('There is already slider named', 'cookingpress') . '<strong> ' . $_POST['slider_name'] . '</strong></p></div>';
            }
        }
    }
}
...
if ( array_key_exists( 'action', $_GET ) && 'remove' == $_GET['action'] && array_key_exists( 'slider', $_GET )  ) {
    ...
    if ( wp_verify_nonce( $_REQUEST['_wpnonce'], 'remove_slider' ) ) {
        ...
        if (  ! empty( $_GET['slider'] ) ) {
            ...
            $new_slider->delete();
            $messages[] = '<div id="message" class="updated"><p>' . __('Slider ', 'cookingpress') . '<strong>' . $_GET['slider'] . '</strong>' . __(' has been successfully removed', 'cookingpress') . '</p></div>';
        } else {
            $messages[] = '<div id="message" class="error"><p>' . __('We have some kind of error, please try again ', 'cookingpress') . '<strong></p></div>';
        }
    }
}
