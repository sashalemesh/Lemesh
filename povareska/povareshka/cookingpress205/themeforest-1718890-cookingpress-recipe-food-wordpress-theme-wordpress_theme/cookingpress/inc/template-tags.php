	...
	if(!empty($metadata)) {
		...
		if (in_array("author", $metadata)) {
			printf(
				...
				sprintf(
					'<span class="author vcard"><a class="glyphicons user url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
		...
		if (!empty($time) && in_array("time", $metadata)) {
			echo '<li class="recipe-time"><span class="glyphicons dashboard">';
			echo $time;
			echo "</span></li>";
		}

		if (!empty($serving) && in_array("servings", $metadata)) {
			echo '<li class="recipe-servings"><span class="glyphicons glass" href="#">  ';
			echo $serving;
			echo "</span></li>";
		}

		if (!empty($level) && in_array("level", $metadata)) {
			echo '<li class="recipe-level"><span class="glyphicons signal" href="#"> ';
		...
		if (in_array("date", $metadata)) {
			printf(
				__( '<li>%1$s</li>', 'cookingpress' ),
				sprintf( '<a href="%1$s" class="glyphicons clock" title="%2$s" rel="bookmark">%3$s</a>',
