...
function style_forms(el) {  
    ...
    el.find(".button").each(function(){
        ...
        if ( $(this).find('br').length ) $(this).css({
            'line-height':'0.9',
            'height':'29px',
            'padding-top':'4px'
        });
...
el.find(".sel_left ul").css({'z-index': 5});
    ...
    el.find(".file").each(function(){
        ...
        $(this).before('<span class="file_left"><span class="file_right"><span class="file_val"></span></span></span><span class="file_but">Загрузить фотографию</span>');
    });	
